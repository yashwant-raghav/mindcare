...
export default function SonicSanctuary({ user, onUpdateStudyMinutes }: SonicSanctuaryProps) {
  ...
  useEffect(() => {
    ...
    if (isPlaying) {
      if (playerMode === "youtube") {
        ...
        timer = setInterval(() => {
          if (playerRef.current && playerReadyRef.current && typeof playerRef.current.getCurrentTime === "function" && typeof playerRef.current.getDuration === "function") {
            try {
  ...
  const initPlayer = (videoId: string) => {
    ...
    // Clear and recreate the player div container to prevent React clash
    if (iframeWrapperRef.current) {
      iframeWrapperRef.current.innerHTML = '<div id="youtube-player-iframe" style="width: 100%; height: 100%;"></div>';
    }
  ...
  const handlePlayPause = () => {
    if (isPlaying) {
      ...
      if (playerMode === "youtube") {
        if (playerRef.current && playerReadyRef.current && typeof playerRef.current.playVideo === "function") {
          try {
            playerRef.current.playVideo();
          } catch (e) {}
        } else {
  ...
  const handleSeek = (percent: number) => {
    ...
    if (playerMode === "youtube" && playerRef.current && playerReadyRef.current && typeof playerRef.current.getDuration === "function" && typeof playerRef.current.seekTo === "function") {
      try {
        const duration = playerRef.current.getDuration();
        if (duration > 0) {
  ...
  useEffect(() => {
    if (playerMode === "youtube" && playerRef.current && playerReadyRef.current && typeof playerRef.current.setVolume === "function") {
      try {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto space-y-4 sm:space-y-6 lg:space-y-8 bg-[#F8F9FA] min-h-screen pb-32" id="sonic-container">
      ...
      >
        <div ref={iframeWrapperRef} className="w-full h-full" />
      </div>
