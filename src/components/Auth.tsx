...
export default function Auth({ onSuccess, onBackToLanding, initialMode }: AuthProps) {
    ...
    if (isLogin) {
      ...
      try {
        const response = await fetch("/api/auth/signup", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email: cleanedEmail, name: fullName, password })
        });
        if (!response.ok) {
          const errData = await response.json();
  ...
  const handleAppleGoogleMock = async () => {
    const cleanedEmail = "alex@mindcare.org";
    try {
      // Try to signup the demo user first
      const response = await fetch("/api/auth/signup", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: cleanedEmail, name: "Alex Henderson", password: "password123", is_pro: true })
      });
      let loggedInUser;
      if (response.ok) {
        loggedInUser = await response.json();
      } else {
        // If signup fails (user exists), log them in
        const loginResponse = await fetch("/api/auth/login", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email: cleanedEmail, password: "password123" })
        });
        if (loginResponse.ok) {
          loggedInUser = await loginResponse.json();
        } else {
