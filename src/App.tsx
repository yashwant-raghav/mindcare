...
export default function App() {
    ...
    if (activeEmail) {
      ...
      fetch(`/api/user?email=${encodeURIComponent(activeEmail)}`)
        ...
        .then(userData => {
          ...
          // Recover moods and journals from backend
          fetch(`/api/moods?email=${encodeURIComponent(activeEmail)}`)
            .then(res => res.json())
            .then(moods => setMoodEntries(moods))
            .catch(() => setMoodEntries(PREPOPULATED_MOODS));
            
          fetch(`/api/journals?email=${encodeURIComponent(activeEmail)}`)
            .then(res => res.json())
            .then(journals => setJournalEntries(journals))
            .catch(() => setJournalEntries(PREPOPULATED_JOURNALS));
        })
        .catch(() => {
          // If server fetch fails, fallback to clearing credentials
  ...
  const recoverUserDatabase = (email: string) => {
    fetch(`/api/moods?email=${encodeURIComponent(email)}`)
      .then(res => res.json())
      .then(moods => setMoodEntries(moods));
      
    fetch(`/api/journals?email=${encodeURIComponent(email)}`)
      .then(res => res.json())
      .then(journals => setJournalEntries(journals));
  };
  ...
  const handleAddNewMoodEntry = async (moodVal: number) => {
    if (!user) return;
    ...
    try {
      const res = await fetch("/api/moods", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: user.email, entry: newEntry })
      });
      if (res.ok) {
      ...
      console.error("Failed to add new mood entry:", err);
    }
  };

  const handleAddNewMoodExtended = async (extendedEntry: MoodEntry) => {
    if (!user) return;
    try {
      const res = await fetch("/api/moods", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: user.email, entry: extendedEntry })
      });
      if (res.ok) {
        setMoodEntries([extendedEntry, ...moodEntries]);
      ...
      console.error("Failed to add extended mood entry:", err);
    }
  };

  // Add Journal Entry
  const handleAddNewJournalEntry = async (newJournal: JournalEntry) => {
    if (!user) return;
    try {
      const res = await fetch("/api/journals", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: user.email, entry: newJournal })
      });
      if (res.ok) {
  ...
  const handleDeleteJournalEntry = async (id: string) => {
    if (!user) return;
    try {
      const res = await fetch(`/api/journals/${id}?email=${encodeURIComponent(user.email)}`, {
        method: "DELETE"
      });
      if (res.ok) {
  ...
  const handleUpdateMindfulMinutes = async (mins: number) => {
    if (!user) return;
    const updatedUser: User = {
      ...user,
  ...
  const handleHardResetDatabase = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setUser(updatedUser);
        alert("MindCare database completely purged.");
      }
    } catch (err) {
